...
module.exports = function() {
  ...
  files.forEach(function generateComponent(svgPath, index) {
    ...
    if (groups[categoryName]) {
      groups[categoryName].push(importName);
    } else {
      groups[categoryName] = [importName];
    }
  });
  Object.entries(groups).map(([categoryName, components]) => {
    content += `\n<h3>${categoryName}</h3>`;
    content = components.reduce((content, component, index) => {
      if (index === 0) {
        content += '\n<ul>';
      }
      content += `\n  <li>
    <${component} />
    <span className="icon-label">${component}</span>
  </li>`;
  ...
  fs.outputFileSync(
    resolvePath(DIST_DIR, 'IconList.tsx'),
    prettier.format(
